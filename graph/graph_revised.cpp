...
//連結とは限らない無向グラフに対し、それを連結成分ごとに分解してそれぞれの頂点集合を返す
    //入力: 無向グラフG
    //出力: 各連結成分ごとの頂点集合のvector /各頂点がどの連結成分に属するかのvector
template<typename T>
pair<vector<vector<T>>, vector<vector<T>>> partitioned_graph(vector<vector<T>>& g){
    int n=g.size();
    vector<int> memo(n,-1);
...
*/
template<typename T>
void dfs_forloop(int s, int& van, vector<int>& seen, vector<int>& finished, vector<int>& ans,
 stack<int>& history, vector<vector<T>>& G){
    if(van==1) return;
    seen[s]=1;
    history.push(s);
    for(auto to:G[s]){
...
template<typename T>
vector<vector<T>> loop_finder(vector<vector<T>>& G){
    int Gsz=G.size();
    vector<int> ans;
...
template<typename T>
void dfs_forloop(int s, int& van, vector<int>& seen, vector<int>& finished, vector<vector<int>>& ans, 
stack<int>& history, vector<vector<T>>& G, int& anscnt){
    if(van==1) return;
    seen[s]=1;
    history.push(s);
    for(auto to:G[s]){
...
template<typename T>
vector<vector<int>> loop_finder(vector<vector<T>>& G){
    int Gsz=G.size();
    vector<vector<int> ans(Gsz);
    vector<int> seen(Gsz,-1);
    vector<int> finished(Gsz,-1);
    int anscnt=0;
    for(int i=0;i<Gsz;i++){
...
*/
template<typename T>
void color_dfs(vector<vector<T>>& G, vector<int>& coloring, int s, int color, int& imp){
    if(imp==1) return;
    coloring[s]=color;
    for(auto to:G[s]){
...
template<typename T>
vector<int> bipartite_coloring(vector<vector<T>>& G){
    int n=G.size();
    int imp=0;
