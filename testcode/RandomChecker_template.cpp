...
vl random_int(ll upper, int n){ //[1,upper]の整数をランダムにn個生成
    vl vec(n);
    rep(i,n){
        vec[i]=Random(upper);
    }
...
int solve(){//適切な入力を設定 以下に解答用のコードを書く
    clock_t start=clock();
    //ここにコードを書く
...
int solve_jury(){//確実に正しい解答を作成するコードを書く
return -100;
}
...
int main(){
    ...
    rep(rp,continue_time){
        ...
        //
       int u1 = solve(); //上で作った適切な入力を入れる
       int u2 = solve_jury();
       if(u1==u2){
        cout<<"WA on case "<<rp<<endl;
        return 0;
       }
    }
