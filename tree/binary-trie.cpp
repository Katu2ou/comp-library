...
<binary trie> (https://kazuma8128.hatenablog.com/entry/2018/05/06/022654)
    ...
    - insert(x) : 値xを集合から一つ挿入
    - erase(x) : 値xを集合から消去
    - max_element/min_element() : 集合内の最大/最小値を取得
    - lower_bound/upper_bound(x) : 集合内でx以上(より大きい)の最小の要素の番号(小さい方から何番目か)を取得

    - kth_element() : k番目に小さい要素を取得
    - max_element/min_element(x) : 集合内の値の中で，値xとXORしたときに最大・最小となる値の取得
                                    (XORした値ではなく，集合の元を返す)
    - kth_element(k, x) : 集合内の値の中で，値xでXORしたときにk番目に小さい値の取得

    - xor_all(x) : 全要素を値xでxorした値に変更 (遅延評価をするとO(1))
