...
uf.find(i) //iを含むグループのroot(代表元)を返す
uf.merge(a,b,c) // a を含むグループと b を含むグループを併合し、b の重みは a と比べて w 大きくなるようにする
uf.diff(a,b) // a と b が同じグループに属す場合、b の重みから a の重みを引いた値を返す
uf.connected(a,b) // a と b が同じグループに属す場合 true, それ以外の場合は false
uf.size(a) // aを含むグループのsize
//