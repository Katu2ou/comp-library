/*
    LARSCHアルゴリズム 
    - Aをn*nの下三角行列でTotally monotoneとしたとき，各iについてargmin(A_ij)を求める
    (Totally monotone: m*n行列Aのi行目の最小要素のうち最も左に位置するものの列番号をf(i)とおくとき，
    f(1)<=...<=f(n)が成り立つことを，monotoneであるという．Aの任意の小行列に対してmonotoneなら，
    Aをtotally monotoneであるという．Aがmonge行列ならAはtotally monotoneである．
    ここで，AがTotally Monotoneであることは，Aの任意の2*2部分行列がmonotoneであることと同値．
    )
    - 入力: そのような行列
    - 出力
