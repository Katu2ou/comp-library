...
/*
  2次元セグメント木
    二次元配列上において非可換なモノイドに関する演算を計算するときに使用できる
    要素の一点更新と二次元領域の総積の取得をO(logHlogW)で行える

    - T データの型  F モノイドを計算する関数の型 
    (int,intを受け取ってintを返す場合は，function<int(int,int)> と書く)
    - SegmentTree2D(h, w, _f, const T& i) _fはモノイドを計算する関数 _i はモノイドの単位元(時空間O(hw))
    - void init(h,w) サイズをh*w, 要素を単位元Iに初期化する
    - void set(h, w, x) 配列をAとしてA_{h,w}の値を更新する(buildの前に呼ぶ)
    - void build() データを構築する (O(hw))
    - T& get(h, w), operator()(h, w) A_{h,w}の値を取得する(O(1))
    - void update(h,w,x) 配列をAとしてA_{h,w}の値を更新する(いつでも呼べて，O(logh logw))
    - void query(h1,w1,h2,w2) 区間[A_{h1,w1}, A_{h2,w2})の総積を取得する
*/  
