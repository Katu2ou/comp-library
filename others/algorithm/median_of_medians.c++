...
int compare = 0;
int pseudomedian(vector<int> &perm, int l, int r, int width){
    if(width>=5){
        int l1 = l+(r-l+1)/5;
        int l2 = l+2*(r-l+1)/5;
        int l3 = l+3*(r-l+1)/5;
        int l4 = l+4*(r-l+1)/5;
        int a = pseudomedian(perm,l,l1-1,l1-l);
        int b = pseudomedian(perm,l1,l2-1,l2-l1);
        int c = pseudomedian(perm,l2,l3-1,l3-l2);
        int d = pseudomedian(perm,l3,l4-1,l4-l3);
        int e = pseudomedian(perm,l4,r,r-l4+1);
        if(perm[a]>perm[b])swap(a,b);
        if (perm[c] > perm[d]) std::swap(c, d);
        if (perm[a] > perm[c]) std::swap(a, c);
        if (perm[b] > perm[d]) std::swap(b, d);
        if (perm[b] > perm[e]) std::swap(b, e);
        if (perm[c]> perm[e]) std::swap(c, e);
        //cout<<l<<" "<<r<<" "<<(perm[b]>perm[c]? b:c)<<endl;
        return (perm[b]>perm[c]? b:c);
    }
    else if(width==1){
        //cout<<l<<" "<<r<<" "<<l<<endl;
        return l;
        ...
        int d = l+3;
        if(perm[a]>perm[b])swap(a,b);
        if(perm[c]>perm[d])swap(c,d);
        if(perm[b]>perm[d])swap(b,d);
        if(perm[a]>perm[c])swap(a,c);
        //cout<<l<<" "<<r<<" "<<(perm[b]>perm[c]? b:c)<<endl;
        return (perm[b]>perm[c]? b:c);
    }
}

int median_of_medians(vector<int> &perm, int k, int l, int r){   
    int pivot = pseudomedian(perm,l,r,r-l+1);
    int lcnt = 0;
    int lind = l;
    ...
    if(lcnt>k-1){
        return median_of_medians(perm,k,l,l+lcnt-1);
    }
    else{
        return median_of_medians(perm,k-lcnt-1,l+lcnt+1,r);
    }
}
...
signed main(){
    ...
    rep(t,iteration){
        int k = rand_int(1,n);
         shuffle(all(base),engine);
         vector<int> perm2 = base;
         vector<int> perm = base;
        clock_t start = clock();
            int u = median_of_medians(perm,k,0,n-1);
        clock_t end = clock();

        double nowtime = double(end - start) / CLOCKS_PER_SEC;
        if(t>=10 && t<=990)chmax(maxtime1,nowtime);
        time1 += nowtime;
        clock_t start2 = clock();
        int v = Kth_small(perm2,k,0,n-1);
        clock_t end2 = clock();

        double nowtime2 = double(end2 - start2) / CLOCKS_PER_SEC;
        if(t>=10 && t<=990)chmax(maxtime2,nowtime2);
        time2 += nowtime2;
        //cout<<"time: "<<nowtime<<endl;
    }
    cout<<"median average: "<<time1/iteration<<endl;
    cout<<"median max: "<<maxtime1<<endl;
    cout<<"random average: "<<time2/iteration<<endl;
    cout<<"random max: "<<maxtime2<<endl;
